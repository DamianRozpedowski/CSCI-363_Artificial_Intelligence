...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        for distribution in livingGhostPositionDistributions:
            maxPosition = distribution.argMax()
            likelyGhostPositions.append(maxPosition)

        nearestGhost = None
        minDistance = float('inf')

        # Determine closest ghost based on distance
        for position in likelyGhostPositions:
            distance = self.distancer.getDistance(pacmanPosition, position)
            if distance < minDistance:
                minDistance = distance
                nearestGhost = position

        # Determine best action for Pacman
        bestMove = None
        bestMoveDistance = float('inf')
        for move in legal:
            futurePosition = Actions.getSuccessor(pacmanPosition, move)
            moveDistance = self.distancer.getDistance(futurePosition, nearestGhost)
